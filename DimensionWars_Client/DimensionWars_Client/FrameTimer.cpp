...
void FrameTimer::Tick(float fLockFPS)
{
	if (m_bStopped)
	{
		m_fTimeElapsed = 0.0f;
...
unsigned long FrameTimer::GetFrameRate(LPTSTR lpszString, int nCharacters) 
{
    if (lpszString)
    {
...
float FrameTimer::GetTimeElapsed() 
{
    return(m_fTimeElapsed);
}

float FrameTimer::GetTotalTime()
{
